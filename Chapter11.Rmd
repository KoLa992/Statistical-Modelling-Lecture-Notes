...
Based on this, the **mathematically formulated statement** regarding the proportion of those who trust the parliament is: $$P_{\text{Believe}} < P_\text{NotBelieve} - 0.02$$

This follows **the principle we saw in Section 2**. If we focus solely on the term *"lower"* in the statement, the inequality would be $P_{\text{Believe}} < P_{\text{NotBelieve}}$. However, due to the *"more than 2 percentage points"* part, we say that even if we subtract 2 percentage points ($0.02$) from $P_\text{NotBelieve}$, $P_\text{Believe}$ remains lower (i.e., smaller). Thus, we arrive at our final inequality that mathematically describes the fundamental statement: $P_{\text{Believe}} < P_\text{NotBelieve} - 0.02$.

If we **rearrange this initial inequality in terms of the difference of proportions**, we get the inequality $P_\text{Believe} - P_\text{NotBelieve} < -0.02$.
...
- $H_0:P_\text{Believe} - P_\text{NotBelieve} \geq -0.02$
- $H_1:P_\text{Believe} - P_\text{NotBelieve} < -0.02$
- Statement: $H_1$

Now comes the second step of hypothesis testing, where we start examining the observed sample data. We **calculate the proportion of those who trust the parliament in both samples ($1=\text{Believe}$ and $2=\text{NotBelieve}$), i.e., $p_1$ and $p_2$!** To do this, we must look at the **total sample sizes** ($n_1$ and $n_2$) and the **number of favorable cases** ($k_1$, $k_2$) in terms of the proportion.
