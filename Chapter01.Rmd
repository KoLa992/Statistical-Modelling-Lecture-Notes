...
Now, let's make R calculate the results of two operations: $3+2$ and $3 \times 2$:
...
The function `str` also shows the current value stored in the object (which is `5` in the example above) next to its data type.
...
We can create R objects containing multiple values with the function `c`. The name of this kind of objects containing multiple values in a sequence is **vector**.
...
If we want to store values of different data types in a vector, then R converts everything to the most general data type. Which is in the example below is `character`:
...
When using the documentation and R in general be careful for the fact that R is a **case-sensitive** language! This means that the difference between small and large letters matter! Like the `order` function is **not** the same as the `Order` function!!
...
On the other hand, if you need to select an element from the list, then you'll need use some tricks since the method applied in the case of vectors does not work here:
...
- PerCapita_HealthExp_2016: per capita health expenditures based on purchasing power parity (2016)
...
```{r}
head(COVID) # shows first 6 rows by default
...
If during package install, R writes WARNINGs on the console with red colour, then these messages can be ignored. The red can be terrifying, but until the message in red doesn't start with *Error*, we're fine! :)
