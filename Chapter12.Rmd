...
UP UNTIL NOW, we examined the case of so-called **parametric tests**. In those, we made statements — hypotheses — about possible population values of statistical indicators, i.e. statistical parameters (e.g., mean, standard deviation, proportion, etc.).
...
So **in the population** (i.e., outside of our sample), **the distribution of job satisfaction levels can be considered IDENTICAL between Windows and non-Windows users**.<br>
...
The `expected` element in the result list returned by `chisq.test` is the contingency table that would occur if $H_0$ were true (i.e., if the two groups were to have the same job satisfaction distribution). We can verify using `prop.table` that in this theoretical contingency table, the proportions within columns = OS groups are indeed equal — just as $H_0$ states.
